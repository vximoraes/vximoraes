...
USER_NAME = os.environ['USER_NAME']  # 'vximoraes'
...
def svg_overwrite(filename, age_data, commit_data, repo_data, contrib_data, loc_data):
    """
    Parse SVG files and update elements with my age, commits, repositories, and lines written
    """
    tree = etree.parse(filename)
    root = tree.getroot()
    justify_format(root, 'age_data', age_data, len(age_data) + 12)
    ...
    justify_format(root, 'loc_del', loc_data[1], 11)
...
if __name__ == '__main__':
    """
    Vinicius Moraes (vximoraes), 2026
    """
    print('Calculation times:')
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2005, 6, 23))
    ...
    svg_overwrite('dark_mode.svg', age_data, commit_data, repo_data, contrib_data, total_loc[:-1])
    svg_overwrite('light_mode.svg', age_data, commit_data, repo_data, contrib_data, total_loc[:-1])

    # move cursor to override 'Calculation times:' with 'Total function time:' and the total function time, then move cursor back
    print('\033[F\033[F\033[F\033[F\033[F\033[F\033[F',
          '{:<21}'.format('Total function time:'), '{:>11}'.format('%.4f' % (user_time + age_time + loc_time + commit_time + repo_time + contrib_time)),
